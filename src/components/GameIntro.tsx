...
import { getSelectedAssetPackId, setSelectedAssetPackId, type AssetPackId } from '@/game/assetPacks';
...
export const GameIntro = ({ onStart }: GameIntroProps) => {
  ...
  const [assetPack, setAssetPack] = useState<AssetPackId>(() => getSelectedAssetPackId());
  ...
  const selectPack = (id: AssetPackId) => {
    setAssetPack(id);
    setSelectedAssetPackId(id);
  };
        ...
        {phase === 'ready' && (
          <div className="space-y-8 animate-fade-in max-w-xl mx-auto">
            ...
            <div className="bg-[hsl(var(--card))]/50 backdrop-blur-sm rounded-lg p-6 border border-[hsl(var(--border))] text-left">
              ...
              <div className="mt-6 pt-4 border-t border-[hsl(var(--border))]">
                <h4 className="font-bold text-[hsl(var(--foreground))] mb-2 font-serif text-sm">ASSET PACK</h4>
                <div className="flex items-center gap-3">
                  <Button
                    type="button"
                    onClick={() => selectPack('hd')}
                    variant={assetPack === 'hd' ? 'abyss' : 'outline'}
                    className="font-serif"
                  >
                    HD
                  </Button>
                  <Button
                    type="button"
                    onClick={() => selectPack('classic')}
                    variant={assetPack === 'classic' ? 'abyss' : 'outline'}
                    className="font-serif"
                  >
                    Classic
                  </Button>
                  <span className="text-xs text-[hsl(var(--muted-foreground))]">
                    {assetPack === 'hd' ? 'Modern visuals' : 'Original look'}
                  </span>
                </div>
              </div>
            </div>
